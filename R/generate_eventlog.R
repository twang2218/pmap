...
#' @param activity_categories A data frame contains the activity category
#' @param activity_categories_size How many activity categories in each activity category
#' @usage generate_eventlog(
#'      size_of_eventlog = 1000, 
#'      number_of_cases = 20, 
#'      activity_categories = c("normal", "visit", "phone", "target"),
#'      activity_categories_size = c(5, 4, 3, 2))
...
#' #  $ activity_category : chr  "campaign" "sale" "campaign" "sale" ...
#' head(eventlog)
...
generate_eventlog <- function(
  ...
  for (i in 1:length(activity_categories)) {
    activities_of_category <- data.frame(
      name = paste0("Activity ", nrow(activities) + 1:activity_categories_size[i], " (", activity_categories[i], ")"),
      category = activity_categories[i],
      weight = stats::runif(activity_categories_size[i], 0, 1),
      stringsAsFactors = FALSE
    )
  ...
  eventlog <- data.frame(
    ...
    activity_category = activities$category,
    stringsAsFactors = FALSE
