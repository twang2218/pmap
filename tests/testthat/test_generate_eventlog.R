...
test_that("generate_eventlog() should be able to generate eventlog of given size", {
  eventlog <- generate_eventlog(
    ...
    number_of_cases = 10,
    activity_categories = c("normal", "target"),
  ...
  expect_equal(nrow(eventlog %>% dplyr::distinct(activity_category)), 2)
  expect_equal(nrow(eventlog %>% dplyr::filter(activity_category == "normal") %>% dplyr::distinct(activity)), 8)
  expect_equal(nrow(eventlog %>% dplyr::filter(activity_category == "target") %>% dplyr::distinct(activity)), 2)
})
